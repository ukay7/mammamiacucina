...
Products belong to one or more categories through category_product. The migration preserves every existing assignment. In the product editor, tick all relevant categories; at least one is required. Admin product filtering matches any assigned category and category counts use the same relationship. Bulk Add category keeps existing assignments; Replace all categories replaces the complete selection. The original category_id remains as a compatible primary category. New Excel products receive General, and repeat imports preserve all category links. A product retains one inventory and media collection regardless of the number of categories. The public product grid uses these assignments for category filters and navigation.

## Dynamic storefront grid

/product-grid now uses active database products with at least one active category. The sidebar lists active categories and counts each assigned active product once per category. All Products counts unique products. AJAX handles category/price filters, sorting, page size, pagination and grid/list mode; links and GET forms also work without JavaScript. Requests are abortable, expose loading/error/retry states, and support browser history. Public images use the first image by media order, skipping videos, and are served only for visible products. Product cards display marketing name and CAD selling price, never supplier/import fields. Cards link to /products/{slug}, a database-backed detail view with a fixed-size image/video gallery and ordered thumbnails. Promotional banners and the sidebar promotional card have been removed. Product images use uniform frames. The header lists all active categories and All Products.

## Storefront cart

Priced products have an Add to Cart hover button and a quantity form on their detail page. AJAX updates the server-session cart, header thumbnails and count; cart quantities and removal also work without a page refresh. Prices come from the database, quantities are limited to 99 per product, and configured stock limits are enforced without reserving or decrementing inventory. Unpriced products use an enquiry link. Checkout saves cash-on-delivery orders with customer delivery details. Delivery charges and tax are confirmed separately. Older sample product pages link to the available database catalogue instead of adding demo products.


## Cash checkout and orders
