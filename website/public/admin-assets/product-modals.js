(() => {
 ...
 if(dialog){
  ...
  document.addEventListener('click',e=>{const link=e.target.closest('[data-product-modal]');if(!link)return;e.preventDefault();dirty=false;save.hidden=true;save.disabled=true;frame.src=link.href;dialog.querySelector('h2').textContent=link.textContent.trim()+' Product';dialog.showModal();});
