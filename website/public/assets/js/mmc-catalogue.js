(() => {
 ...
 async function load(url,push=true){
  ...
  try{
   ...
   const search=content.querySelector('#category-search')?.value||'';
   const scrolls=[...content.querySelectorAll('.mmc-filter-scroll')].map(el=>el.scrollTop);
   content.innerHTML=body.html;
   const searchBox=content.querySelector('#category-search');if(searchBox){searchBox.value=search;filterCategories(search);}
   content.querySelectorAll('.mmc-filter-scroll').forEach((el,i)=>el.scrollTop=scrolls[i]||0);if(push)history.pushState({},'',url);
 ...
 root.addEventListener('change',e=>{if(e.target.matches('.mmc-shop-toolbar select,.mmc-allergy-filter input[type="checkbox"]'))e.target.form.requestSubmit();});
 function filterCategories(value){
  const term=value.trim().toLocaleLowerCase();let count=0;
  content.querySelectorAll('[data-category-name]').forEach(el=>{el.hidden=!el.dataset.categoryName.toLocaleLowerCase().includes(term);if(!el.hidden)count++;});
  const empty=content.querySelector('[data-category-empty]');if(empty)empty.hidden=count>0||!term;
 }
 root.addEventListener('input',e=>{if(e.target.matches('#category-search'))filterCategories(e.target.value);});
