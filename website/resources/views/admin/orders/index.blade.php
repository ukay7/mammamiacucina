...
</td><td>Cash · {{ ucfirst($order->payment_status) }}</td><td><div class="mmc-order-row-actions"><a class="btn btn-outline-primary btn-sm" href="{{ route('admin.orders.show',$order) }}">View</a><a class="btn btn-outline-primary btn-sm" href="{{ route('admin.orders.print',$order) }}" target="_blank" rel="noopener" aria-label="Print order {{ $order->number }}"><i class="fas fa-print" aria-hidden="true"></i> Print</a></div></td></tr>@empty<tr><td colspan="6">No orders found for the selected dates and search.</td></tr>@endforelse</tbody></table>{{ $orders->links() }}</div></div></div>
@endsection
