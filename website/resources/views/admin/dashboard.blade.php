...
@if(auth()->user()->hasAdminPermission('roles.view'))<div class="col-md-6"><div class="panel"><div class="panel-container show"><div class="panel-content"><p class="mmc-eyebrow">ACCESS & PERMISSIONS</p><h2>{{ \App\Models\Role::count() }} user types</h2><p>Control what each team member can see and manage.</p><a class="btn btn-primary" href="{{ route('admin.roles.index') }}">View User Types</a></div></div></div></div>@endif
</div><div class="panel"><div class="panel-hdr"><h2>Your Access</h2></div><div class="panel-container show"><div class="panel-content"><div class="mmc-permission-tags">@foreach(auth()->user()->permissions() as $permission)<span class="badge badge-light">{{ (config('admin.permissions')[$permission] ?? null) ?? $permission }}</span>@endforeach</div></div></div></div>
@endsection
